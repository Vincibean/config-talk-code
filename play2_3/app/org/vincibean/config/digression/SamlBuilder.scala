package org.vincibean.config.digression

import org.pac4j.core.client.Clients
import org.pac4j.core.context.BaseConfig
import org.pac4j.play.Config
import org.pac4j.saml.client.Saml2Client
import org.vincibean.config.digression.SamlBuilder._
...
object SamlBuilder {
  ...
  type AlmostFullConfig =
    EmptyConfig with KeystorePath with KeystorePassword with PrivateKeyPassword with SpEntityId with SamlClientName with CallbackUrl with LogoutUrl with MaxAuthLifetime

  type FullConfig = AlmostFullConfig with IdpMetadataPath
  ...
  private def apply[T <: Config](
      accumulator: SamlConfigsAccumulator): SamlBuilder[T] =
...
class SamlBuilder[T <: SamlBuilder.Config] private (
    acc: SamlConfigsAccumulator) {

  def withKeystorePath(keystorePath: String): SamlBuilder[T with KeystorePath] =
    SamlBuilder(acc.copy(keyStorePath = keystorePath))

  def withKeystorePassword(
      keystorePassword: String): SamlBuilder[T with KeystorePassword] =
    SamlBuilder(acc.copy(keyStorePassword = keystorePassword))

  def withPrivateKeyPassword(
      privateKeyPassword: String): SamlBuilder[T with PrivateKeyPassword] =
    SamlBuilder(acc.copy(privateKeyPassword = privateKeyPassword))

  def withIdpMetadataPath(
      idpMetadata: String): SamlBuilder[T with IdpMetadataPath] =
  ...
  def withSamlClientName(
      samlClientName: String): SamlBuilder[T with SamlClientName] =
  ...
  def withMaximumAuthenticationLifetime(
      maxAuthLifetime: FiniteDuration): SamlBuilder[T with MaxAuthLifetime] =
  ...
  def build()(implicit ev: T =:= FullConfig): Unit = {
    ...
    saml2Client.setMaximumAuthenticationLifetime(
      acc.maximumAuthenticationLifetime.length.toInt)
